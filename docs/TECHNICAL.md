...
How ALG Control works underneath: the hardware protocol, the design, the
safety behaviour and the development setup. For installing and using it, see
the [README](../README.md). For how the hardware protocol was worked out, see
[recon/NOTES.md](../recon/NOTES.md).

The target is the Acer ALG (`AL15G-53`, i5-13420H / RTX 3050) under Linux
Mint / Ubuntu. Everything is written in Go.
...
`alg-control_<version>_amd64.deb` is self-contained: it carries the compiled
programs, so the source tree is not needed to install, reinstall or move to
another machine. From a terminal:

```bash
sudo apt install ./build/alg-control_1.1_amd64.deb
```
...
| `docs/` | this document, and the screenshots the README uses |
